  ...
  if [[ ${CREATE_BASELINE} = false ]]; then
    ...
    for i in ${LIST_FILES} ; do
      ...
      if [[ ! -f ${RUNDIR}/$i ]] ; then
        ...
        if [[ $d -eq 1 && ${i##*.} == 'nc' ]] ; then
          if [[ ${MACHINE_ID} =~ orion || ${MACHINE_ID} =~ hera || ${MACHINE_ID} =~ wcoss2 || ${MACHINE_ID} =~ acorn || ${MACHINE_ID} =~ cheyenne || ${MACHINE_ID} =~ gaea || ${MACHINE_ID} =~ jet || ${MACHINE_ID} =~ s4 || ${MACHINE_ID} =~ aws ]] ; then
